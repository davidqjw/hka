...
def run_pipeline(queries):
    ...
    if my_config["metric_setting"]["llm_judge_setting"]["model_name"] == "openai":
        my_config["metric_setting"]["llm_judge_setting"]["openai"]["openai_setting"][
            "api_key"
    ...
    # pipeline = SequentialPipeline(config=None, prompt_template=None)
...
def main(queries):
    ...
    output = run_pipeline(queries)
    print(output)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--query", type=str, nargs="+", help="Input question(s) for the RAG pipeline"
    )
